...
- **Danish-only UI strings** live in `src/lab-guide/strings.da.ts`. Templates use `{name}` placeholders — substitute with the `format()` helper exported from the same file. Don't inline Danish text into framework components when a string already exists; widgets that have author-facing chrome (placeholder, hint messages) should accept an optional override prop so authors can tailor per-instance.
