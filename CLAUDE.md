...
- **Danish-only UI strings** live in `src/lab-guide/strings.da.ts` and gate-message defaults in `gates.ts`. Don't inline Danish text into components when a string already exists in `strings.da.ts`.
